...
const TrackingPage = () => {
  ...
  const handleTrackOrder = async (trackingNo = trackingNumber, forwardingNo = forwardingNumber) => {
    ...
    const apiUrl = `https://admin.v-winexpress.com/api/tracking_api/get_tracking_data?api_company_id=3&customer_code=superadmin${trackingNo ? `&tracking_no=${trackingNo}` : ''
      }${forwardingNo ? `&tracking_no=${forwardingNo}` : ''}`;
  ...
  return (
 <div className="bg-white font-roboto mt-16 md:mt-24 overflow-x-hidden">
  {/* Banner Section */}
  <div className="relative">
    <img
      alt="Illustration of a delivery truck with a map and location pins indicating tracking"
      className="w-full h-[250px] sm:h-[250px] md:h-[350px] lg:h-[450px] opacity-80 object-cover"
      src={location}
      width={1920}
      height={600}
    />
  ...
  <div className="bg-gray-100 p-6 sm:p-10 rounded-lg flex flex-col md:flex-row items-center justify-center space-y-8 md:space-y-0 md:space-x-10 w-full max-w-7xl">
  <div className="flex-shrink-0 transition-transform transform hover:scale-105 shadow-md shadow-neutral-300 w-full sm:w-auto">
    <img
      alt="Delivery person holding a package with a logistics background"
      className="rounded-lg w-full max-w-full md:max-w-2xl h-auto"
      src={tracking}
    />
  </div>
  <div className="flex-grow p-4 md:p-6 rounded-xl shadow-xl shadow-neutral-300 w-full">
    ...
    <div className="mt-8 p-4 sm:p-8 border border-gray-300 rounded-lg">
      <div className="flex items-center border border-yellow-500 rounded-full px-4 sm:px-6 py-2 overflow-hidden">
        <input
      ...
      <div className="flex justify-center mt-4">
        <button
          className="w-full sm:w-1/2 md:w-1/3 bg-indigo-900 hover:bg-indigo-600 text-white py-2 rounded-full text-sm font-semibold"
